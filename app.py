...
All data loaded from pre-saved CSVs — no live data fetching required.
"""
...
def compute_metrics(rets, cum):
    n = len(rets)
    if n == 0:
        return dict(ann_return=float("nan"), ann_vol=float("nan"), sharpe=float("nan"), mdd=float("nan"))
    ...
    for i in range(1, len(df)):
        ...
        if cr != prev_regime or i == len(df) - 1:
            if prev_regime in show_regimes:
                kw = dict(x0=prev_date, x1=cd, fillcolor=REGIME_COLORS_SOFT[prev_regime],
...
def load_spy_csv():
    ...
    try:
        ...
        st.error("hmm_oos_results_SPY.csv not found. Place it in the same folder as app.py.")
        st.stop()
...
@st.cache_data(show_spinner=False)
def apply_smoothing(raw_regimes_tuple, min_days, smooth_window):
    """Re-apply smoothing filter on top of raw CSV regimes based on sidebar sliders."""
    raw = list(raw_regimes_tuple)
    tmp = smooth_regimes(raw, window=smooth_window)
    tmp = enforce_min_duration(tmp, min_days=min_days)
    return enforce_min_duration(tmp, min_days=min_days)
...
st.sidebar.markdown("---")
st.sidebar.caption("IIT Indore · SPY 2005-2024 · Walk-forward Gaussian HMM · 3 states · Pre-computed OOS results")

# ── LOAD & PREPARE DATA ───────────────────────────────────────────────────────

spy_raw = load_spy_csv()

# Re-apply smoothing based on sidebar (so slider still works interactively)
smoothed = apply_smoothing(tuple(spy_raw["regime"].values), min_days, smooth_window)
spy_raw  = spy_raw.copy()
spy_raw["regime_smooth"] = smoothed

# Date filter
min_date  = spy_raw.index.min().date()
max_date  = spy_raw.index.max().date()
date_range = st.sidebar.date_input("Date range", value=(min_date, max_date),
                                    min_value=min_date, max_value=max_date)
if isinstance(date_range, tuple) and len(date_range) == 2:
    view = spy_raw.loc[str(date_range[0]):str(date_range[1])].copy()
else:
    view = spy_raw.copy()
if len(view) < 5:
    view = spy_raw.copy()

# Rebase cumulative returns to start at 1.0 within the selected date window
# so charts always show growth-of-$1 from the start of the visible period
view = view.copy()
view["strategy_cum"] = (1 + view["strategy_net_ret"]).cumprod()
view["bah_cum"]      = (1 + view["bah_ret"]).cumprod()
...
with tab1:
    ...
    for ds, lbl in {"2008-09-15": "2008 Crisis", "2020-03-23": "COVID Crash", "2022-01-03": "2022 Bear"}.items():
        ...
        if view.index.min() <= d <= view.index.max():
            ...
            fig1.add_annotation(x=d, y=1.02, yref="paper", text=lbl, showarrow=False,
    ...
    fig3.add_trace(go.Scatter(x=view.index, y=dd_series(view["strategy_cum"]), mode="lines",
                               fill="tozeroy", line=dict(color=ACCENT, width=1.2),
                               fillcolor="rgba(79,209,197,0.2)", name="HMM Strategy"))
    fig3.add_trace(go.Scatter(x=view.index, y=dd_series(view["bah_cum"]), mode="lines",
                               fill="tozeroy", line=dict(color="#ef5b5b", width=1.2),
                               fillcolor="rgba(239,91,91,0.15)", name="Buy & Hold SPY"))
    ...
    with col_r:
        ...
        for regime in ["Bull", "Chop", "Bear"]:
            ...
            fig4.add_trace(go.Histogram(x=data, name=regime, opacity=0.55,
        ...
        apply_layout(fig4, height=340, barmode="overlay", xaxis_title="Value", yaxis_title="Density",
                     title=dict(text=f"Feature Distribution per Regime - {FEATURE_LABELS[selected_feature]}",
    ...
    apply_layout(fig5, height=340, yaxis_title=FEATURE_LABELS[selected_feature], showlegend=False,
                 title=dict(text=f"Feature Over Time - {FEATURE_LABELS[selected_feature]}",
                            font=dict(color=TEXT, size=13)))
    st.plotly_chart(fig5, use_container_width=True)

    # Transition Matrix (empirical from smoothed regimes)
    ...
    fig6 = go.Figure(data=go.Heatmap(
        ...
        zmin=0, zmax=1,
        text=[[f"{v:.3f}" for v in row] for row in tp],
    ...
    st.markdown("---")
    st.caption(
        f"Showing {len(view):,} OOS trading days — {view.index.min().date()} to {view.index.max().date()} — "
        f"Min dwell: {min_days}d — Source: hmm_oos_results_SPY.csv"
    )
...
with tab2:
    ...
    if not ca_dfs:
        ...
        st1, st2, st3, st4, st5 = st.tabs([
            "Cumulative Returns", "Regime Timeline", "Regime Alignment",
            "Drawdown Comparison", "Performance Summary",
        ])
        ...
        with st1:
            ...
            for asset, df in ca_dfs.items():
                ...
                fig_cr.add_trace(go.Scatter(
                    x=df.index, y=(df["strategy_cum"] - 1) * 100, name=f"{asset} HMM",
                    line=dict(color=color, width=2),
                    hovertemplate=f"<b>{asset} HMM</b><br>%{{x|%b %d %Y}}<br>Return: %{{y:.1f}}%<extra></extra>"))
                fig_cr.add_trace(go.Scatter(
                    x=df.index, y=(df["bah_cum"] - 1) * 100, name=f"{asset} B&H",
        ...
        with st2:
            ...
            for i, (asset, df) in enumerate(ca_dfs.items(), start=1):
                rc = "regime_smooth" if "regime_smooth" in df.columns else \
                     ("regime" if "regime" in df.columns else
                      [c for c in df.columns if "regime" in c][0])
                for regime, val, color in [("Bull", 1, REGIME_COLORS["Bull"]),
                                            ("Chop", 0, REGIME_COLORS["Chop"]),
                                            ("Bear", -1, REGIME_COLORS["Bear"])]:
                    mask = df[rc] == regime
                ...
                fig_rt.update_xaxes(gridcolor=GRID, linecolor=GRID,
                                     tickfont=dict(color=MUTED), row=i, col=1)
        ...
        with st3:
            ...
            if "SPY" in combined.columns:
                ...
                fig_cond.update_layout(**{**PLOTLY_BASE, "height": 320,
                                          "title": dict(text="Conditional Regime Distribution (Anchor: SPY)",
                                                        font=dict(color=TEXT, size=13))})
        ...
        with st4:
            ...
            for i, (asset, df) in enumerate(ca_dfs.items(), start=1):
                ...
                fig_dd.add_trace(go.Scatter(
                    ...
                    fillcolor=f"rgba({r_},{g_},{b_},0.15)", legendgroup=asset,
            ...
            fig_dd.update_layout(**{**PLOTLY_BASE, "height": 900,
                                    "title": dict(text="Cross-Asset Drawdown: HMM Strategy vs Buy & Hold",
        ...
        with st5:
            ...
            apply_layout(fig_ps, height=360, barmode="group", yaxis_title="Sharpe Ratio",
                         title=dict(text="Sharpe Ratio: HMM vs Buy & Hold Across Assets",
                                    font=dict(color=TEXT, size=14)),
                         legend=dict(orientation="h", y=-0.15, bgcolor="rgba(0,0,0,0)"))
...
with tab3:
    ...
    st.markdown("---")
    st.caption(
        f"Prediction based on empirical transition probabilities from {len(view):,} OOS trading days — "
        f"IIT Indore — HMM Market Regime Classification — Source: hmm_oos_results_SPY.csv"
    )
